...
export default function BulkVoucherPage() {
  ...
  useEffect(() => {
    const fetchExpenses = async () => {
      ...
      if (!idsParam) {
        message.error("No expenses selected");
        setLoading(false);
        return;
      }
  ...
  return (
    <div className="bulk-voucher-container" style={{ padding: "20px", background: "white" }}>
      <style jsx global>{`
        @media print {
          body * {
            visibility: hidden;
          }
          .bulk-voucher-container, .bulk-voucher-container * {
            visibility: visible;
          }
      ...
      <div className="no-print" style={{ marginBottom: 20, textAlign: "right", position: "sticky", top: 0, background: "white", padding: "10px 0", zIndex: 100 }}>
        <Button type="primary" icon={<PrinterOutlined />} onClick={handlePrint} size="large">
          Print All Vouchers ({expenses.length})
        </Button>
      </div>
        ...
        >
          <div style={{ border: "2px solid #000", padding: "30px", minHeight: "600px" }}>
            ...
            >
              ...
              <div>
                <p>
                  <strong>Payee:</strong> {expense.created_by?.full_name}
                </p>
                <p>
                  <strong>Reference:</strong>{" "}
                  {expense.payment_reference || "N/A"}
                </p>
                <p>
