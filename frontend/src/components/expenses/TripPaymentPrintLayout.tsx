...
export function TripPaymentPrintLayout({
  open,
  onClose,
  expenseIds,
}: TripPaymentPrintLayoutProps) {
  ...
  const fetchExpenses = async () => {
    ...
    try {
      ...
      message.error("Failed to load trip expenses");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handlePrint = () => {
    const printContent = document.getElementById("trip-payment-print-content");
    ...
    printWindow.document.write(`
      ...
      <html>
        <head>
          <title>Trip Expense Application</title>
          <style>
            @page {
              size: A4 portrait;
              margin: 10mm;
  ...
  const getApprovalSteps = (expense: ExpenseRequestDetailed) => {
    const steps = [];
  ...
  return (
    <Modal
      title="Trip Expense Application - Print Preview"
      open={open}
      onCancel={onClose}
      width={900}
      centered
    ...
    >
      {loading ? (
        ...
        <div style={{ textAlign: "center", padding: 50 }}>No trip expenses to print</div>
      ) : (
        <div id="trip-payment-print-content">
          {expenses.map((expense, pageIndex) => (
            ...
            >
              ...
              <table className="info-table" style={{ width: "100%", borderCollapse: "collapse", marginBottom: "15px" }}>
                <tbody>
                  ...
                  <tr>
                    ...
                    <td className="value" style={{ padding: "6px 10px", border: "1px solid #ccc" }}>{expense.trip?.route_name || "-"}</td>
                  </tr>
                </tbody>
              </table>
              ...
              <table className="approval-table" style={{ width: "100%", borderCollapse: "collapse", marginBottom: "15px", fontSize: "9pt" }}>
                ...
                <tbody>
                  {getApprovalSteps(expense).map((step, idx) => (
                    <tr key={idx}>
