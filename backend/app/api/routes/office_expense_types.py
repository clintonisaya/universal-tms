...
def create_office_expense_type(
    *,
    ...
    """
    try:
        check_duplicate_name(session, OfficeExpenseType, expense_type_in.name)
...
def update_office_expense_type(
    *,
    ...
    if "name" in update_dict:
        try:
            check_duplicate_name(session, OfficeExpenseType, update_dict["name"], exclude_id=id)
        except DuplicateNameError as e:
            raise HTTPException(status_code=400, detail=str(e))
